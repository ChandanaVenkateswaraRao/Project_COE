 ...
 *
 * Handles:
 * 1. Invalid LaTeX backslash escapes such as \Sigma
 * 2. Invalid \u escapes
 * 3. Raw control characters inside JSON strings
 *    such as literal newlines, carriage returns and tabs
 */
function aggressiveJsonFix(jsonString: string): string {
  ...
  // ------------------------------------------------------------
  // Step 1: Protect already-valid escaped backslashes
  // ------------------------------------------------------------
  result = result.replace(
    /\\\\/g,
    DOUBLE_BACKSLASH_PLACEHOLDER,
  );

  // ------------------------------------------------------------
  // Step 2: Fix invalid JSON escape sequences
  //
  // Valid JSON escapes:
  // \" \\ \/ \b \f \n \r \t \uXXXX
  // ------------------------------------------------------------
  ...
  // ------------------------------------------------------------
  // Step 3: Fix invalid Unicode escapes
  // ------------------------------------------------------------
  result = result.replace(
    /\\u(?![0-9a-fA-F]{4})/g,
    "\\\\u",
  ...
  // ------------------------------------------------------------
  // Step 4: Escape RAW control characters inside JSON strings
  //
  // Gemini can sometimes return literal newlines/tabs inside
  // answer_text instead of escaped \\n / \\t.
  //
  // JSON does not allow those raw characters inside strings.
  // ------------------------------------------------------------
  let fixed = "";
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    // Quote toggles JSON string state
    if (char === '"') {
      fixed += char;
    ...
    // Only modify control characters INSIDE strings
    if (insideString) {
      switch (char) {
        case "\n":
          fixed += "\\n";
        ...
        default:
          // Handle other ASCII control characters
  ...
  // ------------------------------------------------------------
  // Step 5: Restore originally valid double backslashes
  // ------------------------------------------------------------
  result = result.replace(
    new RegExp(
      DOUBLE_BACKSLASH_PLACEHOLDER.replace(
        /[.*+?^${}()|[\]\\]/g,
        "\\$&",
      ),
      "g",
